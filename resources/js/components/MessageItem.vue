<template>
    <div class="message-item">
        <p class="name-item-output">
            {{ message_item.name }}
        </p>
        <div class="message-value">
            <div class="message-value-field">
                <span class="message-item-output">
                {{ message_item.message }}
                </span>
                <span class="time-item-output">
                    {{ new Date(message_item.created_at).getUTCHours()}}:{{new Date(message_item.created_at).getUTCMinutes()}}
                </span>
            </div>
            <svg width="11" height="12" @click="deleteMessage(message_item.id)" class="delete-item" viewBox="0 0 11 12" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
    name: "MessageItem",
    props:['message_item','remove_from'],
    components: {},
    methods: {
        deleteMessage(id)
        {
            axios.delete('http://examn/api/message/delete', {data: { id: id }})
            .then((response) => {
                this.remove_from(id)
            });
        }
